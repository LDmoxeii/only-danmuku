...
- **创建规范**：涉及新聚合实例的命令必须使用聚合工厂 (Mediator.factories.create) 创建对象，不得直接 new
  构造器，确保初始化逻辑与领域事件在工厂内完成。
...
- **生命周期钩子**：`onCreate`/`onUpdate`/`onDelete` 等生命周期方法由中介者框架自动回调，禁止在业务代码中手动调用，避免重复触发事件或破坏事务一致性。
