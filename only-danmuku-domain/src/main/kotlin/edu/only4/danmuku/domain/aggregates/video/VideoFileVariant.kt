 ...
 * @date 2026/01/14
 */
@Aggregate(aggregate = "Video", name = "VideoFileVariant", root = false, type = Aggregate.TYPE_ENTITY, description = "视频文件分辨率档位，")
@Entity
@Table(name = "`video_file_variant`")
@DynamicInsert
@DynamicUpdate
@SQLDelete(sql = "update `video_file_variant` set `deleted` = `id` where `id` = ?")
