 ...
 * @date 2026/01/14
 */
@Aggregate(aggregate = "VideoPost", name = "VideoFilePostVariant", root = false, type = Aggregate.TYPE_ENTITY, description = "视频稿件文件分辨率档位，")
@Entity
@Table(name = "`video_file_post_variant`")
@DynamicInsert
@DynamicUpdate
@SQLDelete(sql = "update `video_file_post_variant` set `deleted` = `id` where `id` = ?")
