...
import edu.only4.danmuku.adapter.domain.translation.video_post_processing.ProcessStatusTranslation.Companion.PROCESS_STATUS_CODE_TO_DESC
 ...
 */
@TranslationType(type = PROCESS_STATUS_CODE_TO_DESC)
@Component
class ProcessStatusTranslation :
    TranslationInterface<String>, BatchTranslationInterface<String> {

    companion object {
        const val PROCESS_STATUS_CODE_TO_DESC = "process_status_code_to_desc"
    }
    ...
    override fun translation(key: Any, other: String): String? {
        ...
        return ProcessStatus.valueOfOrNull(code)?.desc
    }
    ...
    override fun translationBatch(keys: Collection<Any>, other: String): Map<Any, String?> {
        ...
        val codeToDesc: Map<Int, String?> = codes.associateWith { c ->
            ProcessStatus.valueOfOrNull(c)?.desc
        }
