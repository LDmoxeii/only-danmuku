...
class AdminInteractController {
    ...
    fun adminInteractLoadDanmu(@RequestBody request: AdminInteractLoadDanmuku.Request): AdminInteractLoadDanmuku.Response {
        // 调用查询获取弹幕分页列表
        ...
        return PageData.create(
            ...
            list = queryResult.list.map { danmuku ->
                AdminInteractLoadDanmuku.DanmukuItem(
    ...
    fun adminInteractLoadComment(@RequestBody request: AdminInteractLoadComment.Request): PageData<AdminInteractLoadComment.CommentItem> {
        // 调用查询获取评论分页列表
        val queryRequest = VideoCommentPageQry.Request(
            videoNameFuzzy = request.videoNameFuzzy
        )
        request.pageNo?.let { queryRequest.pageNo = it }

        val queryResult = Mediator.queries.send(queryRequest)

        // 转换为前端需要的格式
        val pageData = PageData<AdminInteractLoadComment.CommentItem>()
        pageData.list = queryResult.list.map { comment ->
            AdminInteractLoadComment.CommentItem(
