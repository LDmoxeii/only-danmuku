...
import edu.only4.danmuku.adapter.domain.translation.video_file_upload_session.UploadStatusTranslation.Companion.UPLOAD_STATUS_CODE_TO_DESC
 ...
 */
@TranslationType(type = UPLOAD_STATUS_CODE_TO_DESC)
@Component
class UploadStatusTranslation :
    TranslationInterface<String>, BatchTranslationInterface<String> {

    companion object {
        const val UPLOAD_STATUS_CODE_TO_DESC = "upload_status_code_to_desc"
    }
    ...
    override fun translation(key: Any, other: String): String? {
        ...
        return UploadStatus.valueOfOrNull(code)?.desc
    }
    ...
    override fun translationBatch(keys: Collection<Any>, other: String): Map<Any, String?> {
        ...
        val codeToDesc: Map<Int, String?> = codes.associateWith { c ->
            UploadStatus.valueOfOrNull(c)?.desc
        }
